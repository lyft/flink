...
run sdist.
    ...
    Building the source dist is done in the flink-python directory:
      ...
      pip install dist/*.tar.gz"""
        ...
        LIB_FLINK_SQL_CONNECTOR_HIVE_JAR_NAME = os.path.basename(
            find_file_path(os.path.join(LIB_PATH, "flink-sql-connector-hive-2.3.6_*.jar")))
        ...
        if support_symlinks:
            ...
            copy(os.path.join(OPT_PATH, OPT_SQL_CLIENT_JAR_NAME),
                 os.path.join(OPT_TEMP_PATH, OPT_SQL_CLIENT_JAR_NAME))
            copy(os.path.join(LIB_PATH, LIB_FLINK_SQL_CONNECTOR_HIVE_JAR_NAME),
                 os.path.join(LIB_TEMP_PATH, LIB_FLINK_SQL_CONNECTOR_HIVE_JAR_NAME))
